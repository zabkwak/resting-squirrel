...
app.get(0, '/array', {
	hideDocs: true,
}, (req, res, next) => next(null, [1, 2, 3, 4, 5]));
...
describe('Responses', () => {
	...
	it('calls the endpoint which returns an array with wrapArrayResponse option set to true', (done) => {
		request.get({ gzip: true, json: true, url: 'http://localhost:8080/0/array' }, (err, res, body) => {
			expect(err).to.be.null;
			expect(res.headers['content-type']).to.be.equal('application/json; charset=utf-8');
			expect(res.statusCode).to.equal(200);
			expect(body).to.have.all.keys(['data', '_meta']);
			const { data } = body;
			expect(data).to.have.all.keys(['items', 'count']);
			const { items, count } = data;
			expect(count).to.be.equal(5);
			expect(items).to.be.an.instanceOf(Array);
			expect(items.length).to.be.equal(count);
			expect(items).to.be.deep.equal([1, 2, 3, 4, 5]);
			done();
		});
	});
