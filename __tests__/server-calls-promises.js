...
app.get(0, '/redirect', {
	params: [
		new Param('error', false, Type.boolean),
	],
	redirect: true,
}, async ({ query }) => {
	if (query.error) {
...
describe('Promises', () => {
	...
	describe('GET /0/redirect', () => {

		it('calls the redirect endpoint', (done) => {
			request.get({
				url: 'http://localhost:8090/0/redirect',
				gzip: true,
				json: true,
			}, (err, res) => {
				expect(err).to.be.null;
		...
		it('calls the redirect endpoint with error', (done) => {
			request.get({
				url: 'http://localhost:8090/0/redirect?error=1',
				gzip: true,
				json: true,
			}, (err, res, body) => {
				expect(err).to.be.null;
				...
				expect(message).to.be.equal('Redirect endpoint must return string data.');
				expect(code).to.be.equal('ERR_INTERNAL_SERVER_ERROR');
				done();
			});
		});
	});
